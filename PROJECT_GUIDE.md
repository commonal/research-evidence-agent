...
RunHistory（后续前端迭代）
  查询历史任务；中断任务跨重启恢复需等待 Postgres Checkpointer
```
...
目标 PostgreSQL 同时承载 LangGraph Checkpoint 与产品业务数据，但两者逻辑隔离。当前已接入业务数据，Checkpoint 仍使用 `MemorySaver`：
...
| `run_papers` | 任务与论文关联、显示顺序、相关性分数和匹配关键词/查询 |
| `llm_calls` | operation、provider、model、延迟、Token、缓存和调用状态 |

候选子问题和检索式第一版作为 JSONB 保存在 `research_runs`，只有出现独立筛选、统计或编辑需求后再拆为 `research_subquestions` 和 `search_queries`，避免过早增加表和事务复杂度。
...
当前 `compose.yaml` 先提供 PostgreSQL 17，前端和 FastAPI 仍在宿主机启动；完成 Checkpointer 和部署配置后再将应用服务加入 Compose：
...
#### B. PostgreSQL 业务持久化（后端 MVP 已完成，2026-07-15）

- 引入 SQLAlchemy、Alembic 和 psycopg；
- 建立五张最小业务表与幂等 Repository；
